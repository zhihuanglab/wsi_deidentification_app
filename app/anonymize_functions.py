...
class TiffFile(object):
    def __init__(self, path):
        ...
        if version == 42:
            # Standard TIFF - enable NDPI mode for large files
            if self._file_size > (1 << 32):  # > 4GB
                self._ndpi = True
                if DEBUG:
                    print('Large file detected, enabling NDPI mode.')
        elif version == 43:
            self._bigtiff = True
    ...
    def near_pointer(self, base, offset):
        ...
        if self._ndpi:
            seg_size = 1 << 32
            # Calculate how many 4GB segments to add to get close to base
            segments = (base - offset) // seg_size
            if segments > 0:
                offset += segments * seg_size
                # Check if we need one more segment to get closer to base
                if (base - offset) > (seg_size // 2):
                    offset += seg_size
                # Ensure we don't exceed the base address
                while offset > base:
                    offset -= seg_size
        return offset
